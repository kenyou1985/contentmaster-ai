...
const PROFILES: Record<NicheType, CoverNicheProfile> = {
  ...
  [NicheType.TCM_METAPHYSICS]: {
    styleDna:
      '中医玄学 / 倪师向：宣纸肌理、水墨晕染、朱砂点缀、太极/经络线稿作底纹；庄重沉稳，避免卡通卖萌；封面像讲座海报。',
    schemeAHint: '诊室书房场景：古装剪影、经络挂图、毛笔意象，中景，标题占上三分之一。',
    schemeBHint: '浅麻色底：大号书法感标题，单枚印章图形，极简对称。',
    schemeCHint: '高对比：深色底金色字，针灸铜人剪影特写，光束聚焦。',
    refUploadHint: '可上传讲师形象或品牌主色参考；提示词注明传统中式医疗美学、禁止低龄卡通。',
  },
  [NicheType.FINANCE_CRYPTO]: {
    styleDna:
      '金融投资 / Munger 向：深蓝或炭黑底、细网格或 K 线虚化背景、少量高亮绿/红；专业、冷静、数据感，避免花哨插画。',
    schemeAHint: '虚拟演播室感：人物侧影 + 浮层图表，标题如头条新闻。',
    schemeBHint: '纯色底 + 巨大数字/百分比符号 + 一句反常识 Hook。',
    schemeCHint: '特写眼睛或握拳手势叠在波动曲线上，强侧光，警示感。',
    refUploadHint: '可上传品牌 Logo 色板；提示词强调财经媒体级排版。',
  },
  [NicheType.PSYCHOLOGY]: {
    styleDna:
      '暗黑/清醒心理学向：霓虹暗房、高噪点、单强光源、剪影人脸或裂隙意象；神秘、略带压迫但克制。',
    schemeAHint: '走廊或房间一角，单灯源，人物背影，标题荧光色。',
    schemeBHint: '纯黑底 + 一句质问式大字 + 细线脑轮廓。',
    schemeCHint: '眼部极端特写，红蓝双色光，电影海报比例。',
    refUploadHint: '可上传主色参考；提示词强调心理惊悚纪录片风而非卡通。',
  },
  [NicheType.PHILOSOPHY_WISDOM]: {
    styleDna:
      '哲学智慧 / 禅意向：低饱和青灰、雾、远山、莲、留白；静谧、抽象、文字为主。',
    schemeAHint: '山水极简层叠 + 小舟或人物剪影，标题行书感。',
    schemeBHint: '大面积留白 + 居中一句金句 + 小莲花图标。',
    schemeCHint: '明暗对半构图，撕裂雾中的光，象征顿悟。',
    refUploadHint: '可上传水墨参考；提示词强调东方极简与留白。',
  },
  [NicheType.EMOTION_TABOO]: {
    styleDna:
      '情感禁忌向：烛光、雨窗、剪影、深红与藏青；亲密但禁忌感，避免低俗直给。',
    schemeAHint: '室内烛光双人距离感构图，标题弱化在角落。',
    schemeBHint: '单色深红底 + 白字一句禁忌 Hook + 蜡烛小图标。',
    schemeCHint: '雨中车窗模糊外景 + 手特写，高情绪对比。',
    refUploadHint: '可上传色调参考；提示词强调电影剧照感、成人向情绪片。',
  },
  [NicheType.STORY_REVENGE]: {
    styleDna:
      '复仇故事向：黑红配色、雨夜、刀锋/火焰隐喻、粗衬线标题；戏剧张力、叙事片预告。',
    schemeAHint: '雨夜街道背影或对峙剪影，电影宽画幅。',
    schemeBHint: '黑底血红十字线或断裂符号 + 极简复仇宣言。',
    schemeCHint: '人物半脸阴影分割，眼神特写，高噪点。',
    refUploadHint: '可上传反派/主角剪影参考；提示词强调暗黑剧情片缩略图。',
  },
  [NicheType.GENERAL_VIRAL]: {
    styleDna:
      '新闻热点向：头条大字、红黄警示条、地球/新闻演播室虚化；紧迫、可信、像 breaking news。',
    schemeAHint: '新闻分屏或地图底 + 主持人剪影 + 滚动条元素。',
    schemeBHint: '红黄条 + 超大黑体标题 + 单一图标（火焰/地球）。',
    schemeCHint: '地图热点特写 + 箭头 + 倒计时数字感。',
    refUploadHint: '可上传频道角标颜色；提示词强调新闻台包装。',
  },
  [NicheType.YI_JING_METAPHYSICS]: {
    styleDna:
      '易经命理 / 曾仕强向：棕金、祥云、八卦线稿、书卷；亲民讲座感，避免阴森恐怖。',
    schemeAHint: '讲台与八卦背景，温和人物剪影，标题稳重。',
    schemeBHint: '浅金底深褐字 + 单卦符号居中。',
    schemeCHint: '太极阴阳高对比分割 + 手掐指一算剪影。',
    refUploadHint: '可上传中式纹样参考；提示词强调传统文化讲座封面。',
  },
  [NicheType.RICH_MINDSET]: {
    styleDna:
      '富人思维 / 马云语录向：黑金、城市天际线虚化、聚光灯；成功学但不油腻，留白大气。',
    schemeAHint: '演讲台聚光 + 城市夜景虚化 + 金句标题。',
    schemeBHint: '黑底金边大字 + 极简钻石或阶梯符号。',
    schemeCHint: '侧脸剪影 + 金色轮廓光 + 反问句 Hook。',
