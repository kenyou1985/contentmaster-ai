...
function calculateContentRetention(
  ...
  for (const sentence of origSentences) {
    const coreWords = sentence
      .replace(/^(其实|说真的|说实话|好吧|不过|但是|而且|然后|再说|所以|结果)/g, '')
      .trim().slice(0, 20);
    if (coreWords.length < 8) {
      retainedCount++;
      continue;
    }
    if (polished.includes(coreWords)) {
      retainedCount++;
    }
  }
...
function evaluatePolishingEffectiveness(
  ...
  const contentValid = !retention.hasDeletedContent && retention.retentionRatio >= 0.7;

  // 5. 长度变化（严格：超过±15%视为无效）
  const lengthChangeRatio = originalLen > 0 ? Math.abs(polishedLen - originalLen) / originalLen : 0;
  const lengthValid = lengthChangeRatio <= 0.15;
  ...
  if (!lengthValid) {
    reasons.push(`⚠️ 长度变化过大: ${(lengthChangeRatio * 100).toFixed(1)}%（上限15%）`);
  }

  // ===== 综合判定 =====
  // 硬性要求：内容保留 + 长度合理
  // 核心问题：内容被删 = 无论其他指标多高都必须拒绝
  ...
  if (!contentValid) {
    ...
    isEffective = false;
  } else if (humanWordsAdded === 0 && sentenceVariationRatio < 0.05) {
...
`,
  ...
  // 英文
  en: `You are a professional content editor. Your job is to make the text below sound more human-like by replacing formal/synthetic words with natural ones. You MUST preserve ALL original content.
...
- **The output must have the EXACT same length as the input (±5%)**
- Never delete, truncate, or summarize the content
- Never combine multiple sentences into one
- Never drop any content even if it seems repetitive

### Task: Replace only (do not add or delete)
1. **Replace AI template words** with natural expressions:
...
2. **Slightly vary sentence openings** — don't start consecutive sentences with the same word

## Forbidden
- ❌ Delete or remove ANY content
- ❌ Modify the ending CTA
- ❌ Output text shorter than input
- ❌ Output text longer than input by more than 5%

## Output
Output the rewritten text with the exact same content, just with substituted words. Same length as input (±5%).
...
export async function polishTextForAntiAi(
  ...
  while (attempt < MAX_RETRY) {
    ...
    try {
      ...
      // ===== 内容被删 = 拒绝结果，回退到原文 =====
      if (evaluation.lengthChangeRatio > 0.3 || (polishedLen < originalLen * 0.7)) {
