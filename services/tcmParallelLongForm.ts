...
/** 单次分段流式生成下，单章口播汉字的硬性上限 */
export const TCM_CHARS_PER_SEGMENT_SOFT_CAP = 1300;
...
6. 【字数控制】全文必须严格控制在约 7400–8000 字范围内，不得超过 8000 字。内容如有冗余可小幅压缩，但严禁删减正文。
7. 【结尾 CTA（极其重要）】结尾只输出一个互动引导，格式如下：
   身体是本钱，评论区打上"安康"两字，咱们下期再见。
   禁止输出多个引导词段、禁止输出「好了，讲了这么多...」「记住老祖宗的话...」等多余引导句，结尾只保留上述唯一一句。
8. 只输出合并后的正文，禁止输出任何说明、注释或标记。`;
