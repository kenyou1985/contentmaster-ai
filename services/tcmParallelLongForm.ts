...
- ❌ 「说实在的」开头句式 → 只保留第一次出现
- ❌ 「你们最容易」开头句式 → 只保留第一次出现
- ❌ 「你以为只是」开头句式 → 全文只保留最多2个最完整的
- ❌ 「你说这是不是」开头句式 → 全文只保留第一个
- ❌ 「我今天非/先/要/把话/骂」开头句式 → 全文只保留最多2个最完整的
- ❌ 「好了」单独过渡句（无实质内容）→ 删除
- ❌ 「不是巧合」类转折句 → 全文只保留第一个
- ❌ 「你以为只是天气热」类解释句 → 全文只保留1个最完整的
- ❌ 「光讲道理」开头句 → 全文只保留第一个
- ❌ 「有的人/有些人」开头句式 → 全文只保留最多2个最完整的
- ❌ 「脸色/脾胃/睡眠/火气」症状罗列类段落 → 全文只保留最多2个最完整的
- ❌ 「临床上/诊所里 + 阿伯/太太」案例段落 → 全文只保留2个最完整的
...
- **段落开场去重**：全文中同一类型的段落开头模式不得超过规定次数
  - 「我今天非...」「我今天先...」「我今天把话...」「我今天要...」「我今天骂...」→ 全文只保留最多2个最完整的，其余删除
  - 「你以为...」「你以为只是...」「你以为这...」→ 全文只保留最多2个最完整的，其余删除
  - 「你铁齿」→ 全文只保留第一个，其余全部删除（含"别铁齿"、"还在那里铁齿"）
  - 「我年轻时候也铁齿」→ 全文只保留第一个，其余删除
  - 「我在临床上看过」「我在诊所里看过」「我在诊所里也」→ 全文只保留最多2个最完整的案例段落，其余删除
  - 「你说...」「你说这是不是...」「你说你不信...」→ 全文只保留第一个，其余删除
  - 「我讲到这里，你可能觉得」→ 全文只保留第一个，其余删除
  - 「我跟你讲」「我跟你说」→ 全文只保留第一个，其余删除
  - 「好了」单独句（无后续实质内容）→ 删除
  - 「你们不要笑」开头 → 全文只保留第一个，其余删除
  - 「真有这么明显吗」→ 全文只保留第一个，其余删除
  - 「光讲道理还不够」「光讲理论」→ 全文只保留第一个，其余删除
  - 「真要转顺」「事情说到这里」→ 全文只保留第一个，其余删除
- **语义重复检查**：不同表述但讲同一件事的段落，只保留1个最完整的：
  - 立夏/节气/交运日前后 + 铁齿 + 乱穿 + 乱吃 + 乱睡 → 多个段落只保留最多2个最完整的
  - 脸色发白/发黄/发青 + 脾胃报警 → 只保留最多2个最完整的
  - 白黄收稳/黑蓝下沉/颜色让人更沉 → 只保留最多2个最完整的
  - 铁齿的人最后自己倒霉 → 只保留最多2个最完整的
  - 早睡早起少熬夜/晚上早点收心 → 只保留1个最完整的
  - 洗手/洗手换衣服/把乱气挡外面 → 只保留1个最完整的
  - 「你以为只是天气热一点」类 → 只保留1个最完整的
  - 「有的人/有些人...脸色不对」类 → 只保留最多2个最完整的
...
2. 【关键】删除所有重复的模板句/开场白/套话/结尾语（全文只保留第一次或最后一次）：
   ...
   - "我年轻时候也铁齿，有年交运日偏要去爬山，结果摔了一跤，膝盖肿了半个月" → 只保留第一次
   - "我讲到这里，你可能觉得……不对，等等，我说的是" → 只保留第一次
   - "你这是在作死" → 只保留第一次（任意上下文）
   - "人一乱，家就乱；家一乱" → 只保留第一次
   - "诸位乡亲" → 只保留第一次
   - "我今天必须骂醒你们" → 只保留第一次
   - "你们不要笑，这种事情我见太多了……真的，说不下去了" → 只保留第一次
   - "我跟你讲，有时候我自己在诊所里也……唉算了，不说这个了" → 只保留第一次
   ...
   - "说真的" → 全文只保留最多2次，删除其余所有出现（含"说实在的"、"讲真的"）
   - "我这话难听" → 全文只保留最多2次，删除其余
