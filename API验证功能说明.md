...
### 1. 默认配置更新
- **默认使用 OpenLux AI**: 系统现在默认使用 `https://api.openlux.ai` 作为 Base URL
- **默认模型**: 使用 `gpt-5.6-luna` 模型
- **自动检测**: 如果 API Key 以 `sk-` 开头，自动切换到 OpenLux AI
...
2. 在"API Key"输入框中输入您的 API Key
   - 如果使用 OpenLux AI，输入以 `sk-` 开头的 Key
   - 系统会自动识别并配置
...
- **OpenLux AI API 文档**: https://doc.openlux.ai
- **Google Gemini API**: https://ai.google.dev/

## 注意事项

1. **首次使用**: 建议先验证 API Key，确保可以正常连接
2. **网络问题**: 如果在中国大陆，可能需要使用代理或 OpenLux AI 服务
3. **模型可用性**: 如果 `gpt-5.6-luna` 不可用，系统会自动尝试备用模型 `gpt-5.4-mini`
...
- 提前发现问题，避免浪费时间
- 更友好的错误提示
- 默认使用更稳定的 OpenLux AI 服务

---

**更新日期**: 2026年
**版本**: v3.0 - 更新为 OpenLux AI
