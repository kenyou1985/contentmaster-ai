...
4. **全文只用一只狗的名字**：整篇文章从始至终只使用同一个狗名（如「Bean」「Mochi」「Junie」），不要换名字，不要出现多只不同的狗。所有故事都围绕这一只狗展开，增强连贯性。
...
1. **必须使用第一人称「我」**：整篇文章以「我」为中心，分享自己的真实经历、感受和故事。不是在给读者上课，而是像一个朋友在倾诉。
2. **必须有具体细节**：加入真实发生过的时间、地点、人物、对话、动作。不要泛泛而谈，要有「上周三」「我家狗叫饭团」「聊天记录翻了三遍」这样的具体锚点。
3. **必须打破工整结构**：段落长短不一，段落之间可以有跳跃，有自我打断，有跑题。不要像论文那样层层递进。
4. **必须有口语打断和自嘲**：允许出现「写到这里突然觉得自己在说教」「好，我好像跑题了」「算了不改了」「说实话我也不知道有没有用」这样的句子。
5. **全文只用一只狗的名字**：整篇文章从始至终只使用同一个狗名（如「饭团」「小满」「豆豆」），不要中途换名字，不要出现多只不同的狗。所有故事都围绕这一只狗展开，增强连贯性。
6. **禁止工整的「总分总」结构**：不要用「首先、其次、最后」这种列表式表达，不要人为制造完美对称。
7. **禁止硬广结尾**：中文稿**禁止**使用「请点赞并订阅我的频道」这种营销腔。结尾应该是随意的、自嘲的或开放式的，例如：「写完了。狗在打呼噜，口水流了一地。今天就这儿吧。」「你也可以试试。我也还在学。」
...
2. **必须有具体细节**：时间、地点、事件、对话。不要泛泛而谈，要有具体锚点（例：「上周三凌晨两点」「跟同事发消息后翻了聊天记录三遍」「我家狗叫饭团」）。
3. **必须打破工整结构**：段落长短不一，允许自我打断、跑题。不要「首先、其次、最后」式列表。不要像教科书那样层层递进。
4. **必须加入口语打断和自嘲**：「写到这里突然觉得我在说教」「好，我好像跑题了」「算了不改了」「说实话我也不知道有没有用」「唉，我这人就是这样」。
5. **全文只用一只狗的名字**：整篇文章从始至终只使用同一个狗名（如「饭团」「小满」「豆豆」），不要中途换名字，不要出现多只不同的狗。所有故事都围绕这一只狗展开，增强连贯性。
6. **禁止工整「总分总」结构**：不要预设章节标题，不要刻意对称。
7. **禁止硬广结尾**：**禁止**「请点赞并订阅我的频道」。结尾要随意、自嘲、开放式，例如：「写完了。狗在打呼噜，我睡了。希望你今晚睡个好觉。」
8. **情感表达要有「狼狈感」**：允许脆弱、自嘲、尴尬。不要写得像范文那样正确温暖、无懈可击。
...
## 结尾要求（禁止硬广）
- **英文稿**：结尾用随意、自然的方式收尾，像朋友道晚安。根据文章主题选择对应动物的结尾。**禁止**使用「Please like and subscribe to my channel」。
  - 猫主题：「Anyway, my cat is snoring now. Good night.」「Okay, my cat is asleep on my lap. I should probably move.」「My cat just flopped over. Time to sleep, I guess.」
  - 狗主题：「Anyway, my dog is snoring now. Good night.」「Okay, my dog is asleep against my leg. I should probably move.」「My dog just flopped over. Time to sleep, I guess.」
- **中文稿**：结尾用随意、自然、自嘲的方式收尾。**禁止**「请点赞并订阅我的频道」。推荐方式：
  - 「写完了。狗在打呼噜，我也睡了。希望你今晚睡个好觉。」
  - 「好吧，写到这儿我自己也有点累了。我也在学慢慢来。你也是。不急。」
  - 「就这样吧。猫/狗在睡了，我关灯了。晚安。」
`;
