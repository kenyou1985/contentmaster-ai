...
### 第五段：收尾·功德升华
**此段必须在全文最后位置，且内容必须严格按以下顺序：**
1. 先讲一段收束性的正文（约300-500字）：总结本集核心要点，用一句霸气冷峻的终局判断收束
2. 然后直接进入评论区引导（格式固定）：`大家评论区输入[词1]、[词2]、[词3]、[词4]、[词5]、[词6]之一，咱们下期再见。`
3. 立刻写"下课！"作为全文最后一句，写完即停笔

**格式要求：**
- "大家评论区输入……咱们下期再见。"这段话**只出现在第五段末尾**，全文其他地方禁止出现
- "下课！"**只出现在全文最后一句**，不得在其他任何位置出现
- 第五段正文禁止在中间出现"好了"、"以上就是"等收束引导语后才继续写正文
- 第五段正文内容与前四段保持一致风格（严厉、权威），不要突然变温柔
...
8. **中间段落禁止出现收尾语**："好了"、"以上"、"今天先讲到这儿"、"下课"、"咱们下期再见"等句子只允许出现在第五段末尾，全文其他任何位置禁止出现
9. **各段之间用自然过渡**：段与段之间用口语化的转折句衔接（如"我还没讲完"、"还有一点你们必须知道"），不要用"好了"开头的句子过渡
