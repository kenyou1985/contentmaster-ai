...
第一步：动态赛道识别与风格确立（内部执行，不展示）
在生成分镜前，必须先阅读全文，自动识别文本所属赛道，并锁定视觉基调：
...
第三步：画面内部处理（不展示给用户）
降维隐喻： 凡是枯燥的术语（如：CPI、多巴胺、五行相克、潜意识），必须在画面中用观众一秒能看懂的"物理现象"或"实物"来替代。

文字约束： 画面中尽量用画面讲故事。当且仅当必须出现文字标牌时，请将文字放在双引号中（如："股票暴跌"或"DO NOT ENTER"），其余描述不可滥用双引号。

第四步：输出分镜条目
一旦开始输出，禁止输出任何分析过程，直接按以下格式输出分镜条目：
镜头N
镜头文案:[必须包含该分镜对应的所有原文句子，严格按原文输出，严禁删减或遗漏]
图片提示词:图片背景为[主要场景]，[时间段]，[环境简述]。[构图描述：中心区域 → 主体位置/姿态/动作/情绪 → 核心隐喻或视觉张力表达 → 如何体现该语义块的连续性或对比感]。图片中出现的文字需为"XXX"。
视频提示词:[与图片提示词构图描述相同，去掉"图片背景为……。"的开头，增加运镜或动态趋势的描述，如：镜头缓慢推近/火焰瞬间燃起/光影快速流转]
景别:[特写/近景/中景/全景/微观/宏观]
语音分镜:[旁白/主讲人（标注语气，如：娓娓道来/痛心疾首/专业冷峻/悬疑低沉）]
...
[名称]主讲人
[别名]讲述者/专家
[描述][根据识别出的赛道，自动生成一个符合该领域权威人设的外貌描述。例如：穿西装的金融大牛/穿白大褂的学者/穿中式服装的智者]
