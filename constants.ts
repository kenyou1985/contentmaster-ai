...
2. **极度恐慌**：强调"2026灰犀牛撞击""泡沫破裂""资产归零""滞胀重临"。
3. **时间锚定**：禁止出现"本周/上周/未来X个月/下半年"等时间词。
4. **具体细节**：标题须点出与「国际要闻投喂」或当周宏观相关的具体地域、市场、政权或机构，避免空泛「天下大乱」式废话。
5. **禁忌**：不要固定复用某一历史日期的示例措辞；投喂里有新条目时须优先采纳。

# 示例（仅风格参考，勿逐句照抄；真实选题须对齐当次 RSS 投喂）
- 《能源走廊一旦掐断，油价只是序章——真正的雷在谁的资产负债表上？》
- 《央行还在装睡？某类信贷已经堆成火药桶，散户却以为自己在抄底》
- 《北约内部第一次公开翻脸：你以为的统一战线，在利益面前就是纸》
- 《政府又开始用七十年代的笨办法补贴油价——滞胀幽灵最爱这种政客》
- 《中东导弹乱飞你还在加杠杆追热点？这才是典型的奖励超级反应愚蠢》
...
**字数强制要求：必须输出 8000 字 - 10000 字。** 以 30-40 分钟语音时长为目标（约每分钟 250 字），内容完整收束。

【长文生成结构协议】(必须严格执行，共 6 个模组，禁止省略，禁止概括)

**第一层：引子（约 1000 字）**
- 用最新的新闻事件开场（须紧扣本次生成前抓取的国际 RSS 要闻或选题中的具体事实；若提示词中含「国际要闻投喂」区块须优先引用其中条目）。
- 打破大众认知的幻觉，直接抛出核心反讽："大家都觉得……但这简直愚蠢透顶"。
- 立刻建立芒格的毒舌人设，用一句反问把观众拉入分析框架。
- 引入具体数字：死亡人数、经济损失规模、股价跌幅，给读者当头一棒。

**第二层：扒皮——利益与激励机制（约 2000 字）**
...
**第五层：推演——Lollapalooza 效应（约 1500 字）**
...
- 用芒格的"复利在毁灭时同样可怕"来量化：小概率事件叠加多次后，赔率如何急剧变化。

**第六层：给普通人的锦囊（约 1500 字）**
- 在这样的大变局下，作为一个理性的普通人、投资者，该如何保住自己的财富和常识？
- 给出极度具体的行动指南（不是空话，要有可操作的配置思路）。
- 强调耐心、纪律和反人性的操作——"在别人贪婪时恐惧，在别人恐惧时贪婪"。
- 列出 3-5 条铁律：现金为王还是资产为盾？黄金？农田？能源？做空什么？
- 结尾用芒格式的冷笑话或对华尔街的调侃收束，留给听众余音绕梁的回味。
...
5. **禁忌**：禁止输出任何 Markdown 符号（##, **, -, 数字编号），不要分 Part/节，直接输出纯正文。

【输出格式】
只输出纯文本的第一人称叙述语音文稿，**不要**带章节标题、列表标记、特殊符号、前言后语。
`;
...
4. 绝对禁止任何画面/音乐/场景提示符。
5. 在达到最低字数前（<7500字）禁止出现任何结束语（如“下期再见/下课/散会/今天就到这”）；只有达到字数并完成第九节后，最后一段才可收束。
`;
