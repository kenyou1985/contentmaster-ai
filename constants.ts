...
**触发条件**：用户输入"模式一"
**执行动作**：反推中老年/家庭/财富/人性的核心痛点，直接输出10个极具爆款潜力的视频选题。

**输入**：用户可选择输入选题方向关键词（可选）

**输出格式要求**：
1. **爆款标题**（要求：悬念感+痛点+马云背书感）
2. **切入痛点**（一句话说明击中了受众什么心理）
3. **核心金句**（提炼一句极具传播力的"马云语录"）

*选题方向参考：子女啃老、隔代带娃的算计、亲戚间的嫌人贫怕人富、重病床前的真实人性、老人手里的最后三笔钱等。*

# 格式
只输出10个选题，每行一个选题块（含标题、痛点、金句），不要任何前缀说明、不要Markdown格式、不要列表标记。
`;
