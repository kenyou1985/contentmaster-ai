 ...
 * ✅ 正确版本 - Google Apps Script 自动处理 CORS
 * ✅ 支持 GET 和 POST 请求
 * 
 * 重要说明：
 * - 当部署为"网络应用"且权限设置为"任何人"时，GAS 会自动处理 CORS
 * - 不需要手动设置 CORS headers
 * - ContentService 不支持 .setHeaders() 方法
 * 
 * 部署步骤：
...
function extractTranscript(videoId) {
  try {
    ...
    return {
      success: false,
      error: '未找到字幕。可能原因：1) 该视频没有字幕；2) 字幕被禁用；3) 需要登录'
    };
