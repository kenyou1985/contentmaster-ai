...
export const Tools: React.FC<ToolsProps> = ({ apiKey, provider, toast: externalToast }) => {
  ...
  const cleanScriptOutput = (text: string): string => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/^(镜头文案|圖片提示詞|图片提示词|视频提示词|视频提示詞|景别|景別|语音分镜|語音分鏡|音效)[：:]/.test(line)) {
        cleanedLines.push(lines[i]);
        continue;
      }
  ...
  const isContentComplete = (text: string, mode: ToolMode, originalLength: number): boolean => {
    if (mode === ToolMode.SUMMARIZE) {
      ...
      // 如果没有标签，但包含关键部分（核心主題、YouTube 爆款標題、视频簡介、核心要點等），也认为是完整的
...
5. 请务必使用简体中文输出（包括镜头文案、角色和场景描述部分）。如原文为繁体，请先转换为简体再输出`;
    ...
    if (isYouTube && videoId) {
      systemInstruction += `\n\n⚠️ 重要提示：用戶提供了一個 YouTube 视频链接（视频ID: ${videoId}），同時也提供了转录文本。请直接處理转录文本內容，忽略链接部分。`;
    }
...
**注意**：上述输入包含 YouTube 视频链接和转录文本。请直接處理转录文本內容，忽略链接部分。`
            : `## Input Data
${taskInputText}`;
...
8. **禁止提前收尾（關鍵）**：
   - ⚠️ **一次性输出不可能完成全部內容，系統會自动續寫**
   - 在首次输出時，**嚴禁使用任何收尾語**（如「下課」「散會」「下期再見」等）
   - 保持內容連貫流暢，自然過渡，不要有結束的意思
   - 只有在字數達標後的最終收尾時才使用收尾語
9. **TTS 純淨输出（關鍵）**：
   - 嚴禁输出任何括號內的描述詞，如「（教室的燈光漸漸暗去...）」「（院師猛地一拍驚堂木...）」
   - 嚴禁使用 **、*、__、~~ 等 Markdown 特殊符號
   - 嚴禁输出章節標記、段落編號、說明文字、注釋或元信息
   - 只输出純粹的第一人稱語音文稿內容，適合直接 TTS 配音

## 零解釋输出規則（CRITICAL - 絕對禁止違反）
⚠️ **從第一個字就開始洗稿內容，禁止任何前置內容**
- ❌ 禁止输出：「這裡為您提供...」「以下是改写後的內容」「根據您的要求...」等任何說明文字
- ❌ 禁止输出：「---」「##」「標題：」等任何分隔符、標題、標記
- ❌ 禁止输出：「改写如下：」「洗稿版本：」「最終版本：」等任何引導語
- ✅ 正確做法：直接從故事的第一句話開始输出，零解釋，零標記，純文本

## Output Format
**立即開始输出洗稿內容（使用 ${inputLanguage}），從故事的第一個字開始，不要有任何前置說明、標題、分隔符或解釋。**`;
...
## 原文字數統計
原文共 ${originalLength} 字
目標字數：${targetMinLength}-${targetMaxLength} 字（1.5-2倍扩写）
...
## Constraints
- 扩写後的字數必須達到 ${targetMinLength}-${targetMaxLength} 字（原文的1.5-2倍）。
- 保持原文的語氣（專業、幽默或嚴肅），並融入 ${nicheConfig.name} 的風格特色。
- 不要堆砌無意義的廢話，確保新增內容有實質信息量。
- **禁止提前收尾**：一次性输出不可能完成全部內容，首次输出時嚴禁使用「下課」「散會」等收尾語，保持內容連貫。
- **TTS 純淨输出**：嚴禁输出括號內的描述詞、**、*等特殊符號、章節標記、段落編號、說明文字或注釋。

## Output Format
直接输出扩写後的完整純淨文章（使用 ${inputLanguage} 输出），保持簡潔連貫流暢，無需分段標記或元信息。嚴禁使用「## 」「### 」「第一章」「（）」「**」等標記。`;
...
## Goals
從 ${nicheConfig.name} 領域專家的視角，為上述文本生成完整的 YouTube 视频內容包裝方案，包括標題、簡介、標籤和封面設計方案。

## Output Requirements（必须简体中文输出）
...
【標籤語言規則】
- ⚠️ 標籤語言必須與文案內容語言一致
- 如果文案是简体中文，标签必须全部使用简体中文（如 #倪海厦 #中医玄学 #风水）
...
## Output Format
请严格按照上述格式输出，使用简体中文，无需额外解释或分析。严禁使用 **、*、__、~~ 等 Markdown 特殊符号。`;
...
7. **字數保持**：润色後的字數應與原文相當（約 ${originalLength * 0.9}-${originalLength * 1.1} 字），不要大幅縮減或擴充。
8. **禁止提前收尾**：首次输出時嚴禁使用「下課」「散會」等收尾語，保持內容連貫流暢。
9. **TTS 純淨输出（關鍵）**：嚴禁输出括號內的描述詞、**、*等特殊符號，只输出純粹的第一人稱語音文稿。

## Comparison Standard
在"信（準確）、達（通順）、雅（優美）"三個維度上都必須有明顯提升，同時保持 ${nicheConfig.name} 的專業風範。

## Output Format
请直接输出润色後的純淨最終版本（使用 ${inputLanguage} 输出），保持簡潔連貫流暢，無需標註修改痕跡或解釋。嚴禁使用「## 」「### 」「修改說明：」「（）」「**」等任何標記。`;
...
⚠️ **输出完场景信息后，立即结束，不要输出任何其他内容。**`;
    ...
    try {
        ...
        if (isYouTube && (errorMsg.includes('网络') || errorMsg.includes('API Key') || errorMsg.includes('連接'))) {
            ...
            // 显示详细的错误信息
            updateTask({ outputText: `❌ 生成內容時发生错误\n\n错误信息：${errorMsg}\n\n请检查：\n1. API Key 是否正確配置\n2. 网络連接是否正常\n3. API 服务是否可用\n\n如果问题持续，请联系技术支持。` });
        }
    } finally {
    ...
    <div className="max-w-5xl mx-auto space-y-6">
       ...
       <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-slate-800/50 p-4 rounded-xl border border-slate-800">
           <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 w-full md:w-auto">
                {/* Tool Modes */}
                {[
                    { id: ToolMode.REWRITE, label: '改写/洗稿', icon: <RefreshCw size={16} /> },
                    { id: ToolMode.EXPAND, label: '深度扩写', icon: <Maximize2 size={16} /> },
                    { id: ToolMode.SUMMARIZE, label: '摘要总结', icon: <Scissors size={16} /> },
                    { id: ToolMode.POLISH, label: '润色优化', icon: <FileText size={16} /> },
                    { id: ToolMode.SCRIPT, label: '脚本输出', icon: <Video size={16} /> },
                ].map((tool) => (
                    <button
                        key={tool.id}
               ...
               {mode === ToolMode.SCRIPT && (
                 <div className="flex items-center gap-2">
                   ...
                   <select
                     ...
                     className="bg-slate-900 border border-slate-700 rounded-lg px-2 py-2 text-sm text-slate-200 font-bold focus:outline-none focus:border-emerald-500 cursor-pointer"
                   >
      ...
      {/* Grid: Input and Output */}
       <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-[600px]">
          ...
          <label className="text-sm font-medium text-slate-400 flex items-center justify-between">
            ...
            placeholder="请在此粘貼您的內容或 YouTube 链接..."
