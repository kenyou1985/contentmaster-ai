...
直接输出扩写後的完整纯净文章（使用 ${inputLanguage} 输出），保持簡潔連貫流暢，無需分段标记或元信息。严禁使用「## 」「### 」「第一章」「（）」「**」等标记。`;
        case ToolMode.SUMMARIZE: {
                const tlWpmLow = 200;
                const tlWpmHigh = 250;
                const tlMinDur = originalLength / tlWpmHigh;
                const tlMaxDur = originalLength / tlWpmLow;
                const tlMidDur = (tlMinDur + tlMaxDur) / 2;
                const videoIntroLine =
                    taskNiche === NicheType.YI_JING_METAPHYSICS
                        ? '本視頻內容源自曾仕强教授的智慧传承，开场就要提醒你，[開場鉤子 - 製造危機感/緊迫感]'
                        : '本視頻內容源自倪師智慧傳承，[開場鉤子 - 製造危機感/緊迫感]';
                return `### 任务指令：YouTube 内容摘要与优化

${inputSection}

## Goals
從 ${nicheConfig.name} 領域專家的視角，為上述文本生成完整的 YouTube 视频内容包裝方案，包括标题、簡介、標籤和封面設計方案。

## 口播稿时长与时间轴（必严格遵守）
- 以上输入口播稿约 **${originalLength}** 字（以实际字数为准）。
- 配音语速按 **200–250 字/分钟** 估算：整片时长约 **${tlMinDur.toFixed(1)}–${tlMaxDur.toFixed(1)} 分钟**（取中值约 **${tlMidDur.toFixed(1)} 分钟** 亦可）。
- **精彩时间轴导航**（👇）中每条 **MM:SS** 必须从 **00:00** 起按章节顺序递增；各章节起始时间须与文稿结构、篇幅比例一致，且**不得超过**上述总时长上限（最后一章的起始时间仍须小于推算的总时长）。
- **禁止**编造与当前稿长明显不符的超长时间轴（例如输入仅数千至一万余字却出现 **50–60 分钟** 量级的节点）；若推算总时长约 **${tlMinDur.toFixed(1)}–${tlMaxDur.toFixed(1)} 分钟**，则时间轴应落在该范围内（如 8000 字、按 250 字/分钟时整片约 32 分钟，则不应出现 **58:10** 这类节点）。
...
YouTube 爆款标题（5個，必須标题黨風格，留鉤子，用不同角度）：
⚠️ **标题要求（CRITICAL）**：
- 必須是标题黨風格，強烈吸引點擊
- 每個标题必須留鉤子（悬念、反轉、驚人真相、禁忌話題等）
- 5個标题必須從不同角度切入，避免重複
- 可以使用：數字衝擊、疑問句、反轉、禁忌、驚人真相、對比、時間緊迫感等技巧
- 長度：40-60字，要足夠吸引人

1. 【标题1 - 角度1：使用数字冲击/惊人真相/禁忌话题，留强烈钩子】
2. 【标题2 - 角度2：使用疑问句/反转/对比，制造悬念】
3. 【标题3 - 角度3：使用时间紧迫感/警告/危机感，引发焦虑】
4. 【标题4 - 角度4：使用反转/颠覆认知/颠覆常识，制造冲突】
5. 【标题5 - 角度5：使用揭秘/内幕/隐藏真相，满足好奇心】

视频簡介：
${videoIntroLine}

🔥 [日期]（[干支日]/[農曆日期]）不僅僅是一個普通的日子，它是[節氣]前夕天地陰陽瘋狂交戰的「[名稱]」！

[主段落 - 強調目標群體 + 核心問題 + 為什麼重要]
...
[時間4] [章節標題4]
[時間5] [章節標題5]
[時間6] [章節標題6 — 如结尾；章节数须与上文结构一致，可增删行数但时间须自洽]
...
【免責聲明】 本頻道內容僅供參考與教育用途，[領域]傳播[領域名]文化與[相關領域]理念。視頻中提到的建議不能替代專業的醫療診斷或治療。如有身體不適或嚴重疾病，請務必諮詢專業醫師或前往醫院就診。

熱門標籤：
#標籤1 #標籤2 #標籤3 #標籤4 #標籤5 #標籤6 #標籤7 #標籤8 #標籤9 #標籤10 #標籤11 #標籤12 #標籤13 #標籤14 #標籤15

【標籤語言規則】
- ⚠️ 標籤語言必須與文案内容語言一致
- 如果文案是简体中文，标签必须全部使用简体中文（如 #倪海厦 #中医玄学 #风水）
- 如果文案是簡體中文，標籤必須全部使用簡體中文
- 如果文案是繁體中文，標籤必須全部使用繁體中文
- 如果文案是英文，標籤才使用英文
...
AI 圖片提示词（5個，必須使用中文输出）：
⚠️ **提示词要求（CRITICAL）**：
- 必須使用中文输出，禁止使用英文
- 適合 Midjourney/Stable Diffusion/DALL-E 等 AI 繪圖工具
- 描述要具體、视觉化，包含色彩、構圖、光線、氛圍等元素
- 每個提示词要突出不同的视觉重點

1. [提示词1 - 用中文描述封面视觉元素，強調核心概念和情緒張力]
2. [提示词2 - 用中文描述，突出关键人物或場景的视觉特徵]
3. [提示词3 - 用中文描述，展現衝突或對比的视觉效果]
4. [提示词4 - 用中文描述，營造悬念或神秘感的视觉氛圍]
5. [提示词5 - 用中文描述，結合标题文案的视觉呈現]
...
【封面設計要求】
- 圖片提示词要具體描述视觉元素、色彩、構圖、風格
- 封面标题文案必須從内容核心提煉，每行不超過10個字
- 标题文案要簡潔有力，具有视觉衝擊力
