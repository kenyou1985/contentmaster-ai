...
/**
 * 新闻热点·小美赛道收尾铁律：
 * - 正文中若已经出现「咱们下期见」/「咱们下期继续拆」/「咱们下期继续聊」等收尾语，保留并清理尾部多余空白后返回
 * - 若正文不完整收尾（无标点或被 maxChars 截断在句中），自动追加收尾语
 * - 若正文末尾是不完整句子（最近 200 字内无任何句号/感叹号/问号），截到最近的完整句子再加收尾
 */
function ensureNewsShortClosing(text: string): string {
  // 频道「盒子里的秘密」范文规定的固定收尾句（用户 8-19 反馈的范文明示）
  // 此前错误地用「咱们下期见」，违反范文规定，统一修正为「对此你又有什么看法？欢迎在评论区留言。」
  const CANONICAL_CLOSING = '对此你又有什么看法？欢迎在评论区留言。';
  const trimmed = text.trimEnd();
  // 已有的收尾语（包括范文新收尾 + 旧的「咱们下期见」变体）—— 检测到就视为已有收尾
  const closingPhrases = [
    /对此你又有什么看法？\s*欢迎在评论区留言。\s*$/,
    /咱们下期见[。！]?\s*$/,
    /咱们下期继续拆[。！]?\s*$/,
    /咱们下期继续聊[。！]?\s*$/,
    /我们下期见[。！]?\s*$/,
  ...
  // 1. 已有收尾语：清理+补全（统一为范文句）
  for (const re of closingPhrases) {
    if (re.test(trimmed)) {
      // 不管是哪种收尾，都强制替换为范文规定句
      return CANONICAL_CLOSING;
    }
  }

  // 2. 末尾无标点 → 视为被截断，找最近 200 字内最近的完整句子边界
  const lastChar = trimmed.slice(-1);
  const hasTrailingPunct = /[。！？]/.test(lastChar);
  if (!hasTrailingPunct) {
    const searchWindow = trimmed.slice(Math.max(0, trimmed.length - 200));
    ...
    if (sentenceEnd > -1) {
      ...
      return `${prefix} ${CANONICAL_CLOSING}`;
    }
  }

  // 3. 末尾是完整句子但无收尾语 → 追加范文规定收尾
  return `${trimmed} ${CANONICAL_CLOSING}`;
}
...
function getOneShotScriptPlan(niche: NicheType, scriptLengthMode: 'LONG' | 'SHORT' = 'LONG', customMinChars?: number | null): OneShotScriptPlan | null {
  ...
  if (niche === NicheType.GENERAL_VIRAL && scriptLengthMode === 'SHORT') {
    return {
      minChars: 1000,
      maxChars: 1500,
      targetLabel: '1000 - 1500（约 1200 字起收尾）',
      extraDirective:
        ...
        '收尾必须在观点表达完整、句子有明确结论之后，再追加「对此你又有什么看法？欢迎在评论区留言。」，不要为了凑结尾而砍掉正在论证的论点。' +
...
function buildOneShotLongScriptPrompt(
  basePrompt: string,
  topic: string,
  niche: NicheType,
...
【输出格式（必须严格遵守·大小写必须一致）】
- 所有段落标签必须使用**大写 P**：【Phase 1】【Phase 2】【Phase 3】【Phase 4】【正文】【稿件统计】【素材引用说明】——禁止使用小写 phase，禁止中英文混用。
...
**一、内容质量（质检报告要求必须逐条解决）**
1. **素材来源不足**：质检报告的「缺什么」和「缺失来源」列出了具体缺口。终稿必须用「据报道」「据官方公告」「据……机构」等泛化措辞补上这些空白，不编造具体链接，但要让读者知道有据可查。
2. **事实核实**：质检报告列出的关键数字、时间节点（如"45万至50万""2024年5月18日""征兵年龄27岁至25岁"）必须全部保留并准确使用，不得自行替换或删除。
3. **去除有争议的定性词**：质检报告指出「语言风险」中的词汇（如"号称为自由而战""收割底层""交货""消耗品"）必须删除或改为中性表述。终稿立场可以批判，但措辞要有数据支撑，避免情绪化的预设判断。

**二、AI 味清除（质检报告列出的一律删除）**
4. **删除所有残留模板词**（质检报告 Phase 3 列出的，一个不留）。
5. **删除所有说教腔**："各位看官"、"号称为自由而战的国家，开始靠恐惧把人送进战场"、"战争机器……露出了最粗糙的齿轮"、"共同承担，而不是穷人承担得更多"、"不能一边要求普通人……一边把……当成装饰"、"这不是军队强大的标志，而是国家治理的警报"、"国家安全不是一张可以盖住所有问题的红章"、"武力能控制身体，不能制造忠诚"、"只会收索取的机器"、"只收割底层的游戏"、"究竟是在守护共同体，还是在把共同体变成一台只会索取的机器"。
6. **彻底删除纠正式讲解腔**："你以为…？…其实…"、"你以为…错…"、"我们别被…骗了"、"先看美国"、"再看莫斯科"、"别被…哄住"、"谈判是一回事，X是另一回事"。

**三、重复控制**
7. **减少重复比喻**：同一套比喻（棋局/家庭/修仙/账本）整篇最多保留 1 个，其余改写成具体动作或场景。4 类隐喻合计不超过 4 处。
8. **减少重复句式**：整篇最多出现 2 次"不是A，而是B"；最多出现 2 次"一边A，一边B"；最多出现 2 次"表面是A，深处是B"。
9. **减少重复论点**：同一论点只展开一次，不得换说法重讲超过 2 次。

**四、收尾质量（最高优先级）**
10. **禁止生硬收尾**：终稿最后一段必须从论证自然推导，不是突然加上去的感叹段落。不要在论证结束后突然插入"总而言之……"。
11. **「对此你又有什么看法？欢迎在评论区留言。」自然出现**：这是频道「盒子里的秘密」的固定收尾句。在论证完整、结论已给出之后，作为最后一个句子出现，前面紧跟结论句。不要先写互动引导段再接范文收尾，不要在中间突然跳到互动引导破坏论证流畅度。
12. **收尾句数限制**：从范文收尾往前数，前面最多有 2 句议论/结论/互动引导。不允许在正文末段塞入 4-5 句后再写范文收尾。
13. **结尾禁止出现**："各位看官"、"咱们下期见"——只保留范文规定的「对此你又有什么看法？欢迎在评论区留言。」

**六、句式密度控制（最严格·违反即扣分）**
16. **禁止「不能只/尚不足以/判断关键不在/更关键的是/更深一层/这说明/这才是/更要的是」等高频判断短语**——这些短语每篇只能用 1 次。全文「不能只」「尚不足以」「判断关键不在」这类判断短语合计 ≤2 次。
17. **禁止同一概念反复用不同词重述**：「证据不足」「证据不公开」「证据缺失」「证据缺口」——选一个准确的说法，全篇只表达一次。
18. **禁止「止血带、止血、灭火、火扑灭、止血纱布」同组连续使用**——这是用户上次反馈里特别点名的"止血系列比喻过密"，全篇同组比喻最多保留 2 个，其余改写或删除。
19. **禁止「金丹、外门弟子、修仙界、渡劫」等修仙类比**——用户上次反馈明确指出与战争纪实语境不匹配，全部改成具体动作或场景。
20. **禁止「亲爹、后妈、递纸巾、家里人、餐桌」等家庭比喻与棋局/政治比喻叠加**——同段内只允许 1 类隐喻，不混着用。
...
function clampOneShotLength(
  ...
  // 在 [0, maxChars] 区间内找最后一个完整句子边界（中英文标点 + 段落分隔）
  const findSentenceCut = (slice: string): number =>
    Math.max(
      slice.lastIndexOf('。'),
      slice.lastIndexOf('！'),
      slice.lastIndexOf('？'),
  ...
  let truncated = text.slice(0, plan.maxChars);
  let cutoff = findSentenceCut(truncated);

  // 兜底1：如果 maxChars 区间内没有任何句子边界，往前最多再扩 150 字继续找
  if (cutoff < 0) {
    const extended = text.slice(0, Math.min(text.length, plan.maxChars + 150));
    const extCut = findSentenceCut(extended);
    if (extCut >= Math.max(plan.minChars - 1, 0)) {
      truncated = extended;
      cutoff = extCut;
    } else {
      // 实在找不到：保留到 minChars 边界（截到中间也无大碍，下游 ensureNewsShortClosing 会再处理）
      return truncated.trim();
    }
  }

  // 兜底2：如果 cutoff 落在 minChars 之前（极端情况）：找 minChars ~ maxChars 之间的第一个句子边界
  if (cutoff < Math.max(plan.minChars - 1, 0)) {
    const searchRange = text.slice(Math.max(0, plan.minChars - 1), plan.maxChars);
    const idxInRange = findSentenceCut(searchRange);
    if (idxInRange > -1) {
      cutoff = Math.max(0, plan.minChars - 1) + idxInRange;
      truncated = text.slice(0, Math.min(text.length, plan.maxChars));
    }
  }
  ...
  if (cutoff >= 0) {
    ...
    return `${truncated.slice(0, cutoff + 1).trimEnd()}${needsPunct ? '。' : ''}`.trim();
  }

  return truncated.trim();
}
function shouldUseOneShotLongForm(niche: NicheType, scriptLengthMode: 'LONG' | 'SHORT'): boolean {
  // 新闻热点赛道：长短视频都走一次性直出（禁止分段并行+合并）
  if (niche === NicheType.GENERAL_VIRAL) return true;
  // 短视频一律走一次性直出（不分段），对齐抖音短脚本"一口气生成一稿完事"的需求
  if (scriptLengthMode === 'SHORT') return true;
...
function applyGenericVideoClosing(text: string): string {
  ...
  const CANONICAL_CLOSING = '对此你又有什么看法？欢迎在评论区留言。';
  const REPEAT_CLOSING = /(?:对此你又有什么看法？\s*欢迎在评论区留言。|咱们下期见|我们下期见|咱们下期继续聊|咱们下期继续拆)[。！？]*\s*/gi;

  // 清除所有已存在的收尾语（循环清除直到没有）
  let cleaned = base;
  let iters = 0;
  ...
  // 强制以范文规定的收尾句结尾（不依赖模型自己写）
  return cleaned ? `${cleaned} ${CANONICAL_CLOSING}` : `${CANONICAL_CLOSING}`;
}
...
export const Generator: React.FC<GeneratorProps> = ({ apiKey, provider, toast: externalToast }) => {
  ...
  const [planTopicCountCustomValue, setPlanTopicCountCustomValue] = useState('10');
...
- Do not output pure English only. Do not output pure Chinese only. Every line must be bilingual.`;
    ...
    if (isRssNiche) {
      // 抖音热点赛道（国内民生/社会/全网热搜）：使用国内数据源
      const isDouyinHot =
        niche === NicheType.GENERAL_VIRAL && newsSubMode === NewsSubModeId.DOUYIN_HOT;
      const rssLang: 'en' | 'zh' | 'cn-domestic' = isDouyinHot
        ? 'cn-domestic'
        : niche === NicheType.GREAT_POWER_GAME
          ? (greatPowerLanguage === 'en' ? 'en' : 'zh')
          : 'en';
      toast.info(
        isDouyinHot
          ? '正在抓取国内热点（网易/凤凰/央视/新浪）…'
      ...
      try {
        ...
        if (niche === NicheType.GENERAL_VIRAL) {
          ...
          if (newsSubMode === NewsSubModeId.TAIWAN_STRAIT) {
            ...
            const taiwanRules =
              '\n\n【台湾地区时政·社会·财经·民生全维度选题铁律·最高优先级】\n' +
              '**比例铁律（必须严格遵守，违反视为不合格）**：\n' +
              '- 10 条选题中 **≥5 条必须是非政治议题**（财经/产业/食安/民生/健保/教育/劳动/科技/能源/环境/文化/体育/治安/司法案件等）\n' +
              '- 10 条选题中 **≤3 条是对外议题**（两岸/台美/台日，但必须从台湾岛内视角切入）\n' +
              '- **10 条中至多 1 条围绕"美对台军购/军售/防务预算/采购装备"**（典型雷同高发区）\n' +
              '- **10 条中至多 2 条围绕"军事演习/军事部署"**（典型雷同高发区）\n' +
              '- 10 条中至少 **6 种不同事件类型**（议会冲突+食安+台积电+台风+罢工+弊案+选举+社会案件+科技+体育+文艺等不同组合）\n' +
              '- 10 条中至少 **6 个不同政治人物**（如涉及人物）或 **5 种不同新闻主线**\n' +
              '\n**禁止跑偏**：\n' +
              '- 禁止输出伊朗/俄罗斯/乌克兰/中东/南海等与台湾无关的国际新闻\n' +
              '- 禁止输出中美博弈/G7峰会/四方对话等大国政治角力内容\n' +
              '- 禁止变成纯军事台海博弈赛道（应聚焦台湾内部政治反应）\n' +
              '- 禁止"政治+军购"组合独大：若投喂中含大量非政治新闻（财经/民生/科技/社会/食安/劳动/环境/文化等），必须从中取材\n' +
              '\n**多样性铁律**：10 条标题至少覆盖 6 个不同政治人物，6 种以上事件类型，禁止 10 条都围绕同一人物或同一议题（特别是军购/军演）。\n' +
  ...
  const handleBatchGenerate = async () => {
    ...
    if (shouldUseOneShotLongForm(niche, scriptLengthMode)) {
      if (selectedTopics.length !== 1) {
        toast.warning(
          scriptLengthMode === 'SHORT'
            ? '短视频模式一次性直出，请只勾选 1 个选题'
            : '当前长视频赛道已改为一次性生成，请只勾选 1 个选题'
        );
        return;
      }
    ...
    const generationPromises = selectedTopics.map(async (topic, index) => {
        ...
        if (subModeConfig && subModeConfig.scriptPromptTemplate) {
            // 新闻热点赛道：根据时长模式切换抖音短/长视频模板
            if (niche === NicheType.GENERAL_VIRAL) {
                scriptTemplate = scriptLengthMode === 'SHORT'
                    ? NEWS_SHORT_SCRIPT_DOUYIN
                    : NEWS_LONG_SCRIPT_DOUYIN;
            } else {
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
        ...
        {(niche === NicheType.TCM_METAPHYSICS || niche === NicheType.FINANCE_CRYPTO || niche === NicheType.PSYCHOLOGY || niche === NicheType.PHILOSOPHY_WISDOM || niche === NicheType.EMOTION_TABOO || niche === NicheType.YI_JING_METAPHYSICS || niche === NicheType.HISTORICAL_FIGURE || niche === NicheType.GENERAL_VIRAL) && (
                ...
                <div className="flex justify-between items-center mb-3">
                    ...
                    <span className="text-sm text-emerald-400 font-medium">
                      {shouldUseOneShotLongForm(niche, scriptLengthMode)
                        ? `已选 ${topics.filter(t => t.selected).length} 个（一次成稿只勾 1 个选题）`
